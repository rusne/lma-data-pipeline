...
def validate(dataframe, level):

    # import NACE-EWC validation
    # pairs of valid activity-waste codes
    nace_ewc = pd.read_csv("Public_data/NACE-EWC.csv", low_memory=False)
    ...
    eural_sbi = pd.merge(euralcodes, dataframe[['LMA_key', 'KvK_sbi', 'KvK_ag']], on='LMA_key')
    eural_sbi['KvK_sbi'] = eural_sbi['KvK_sbi'].astype('int64')
    eural_sbi['KvK_sbi'] = eural_sbi['KvK_sbi'].astype(str).str.zfill(4)
    eural_sbi['LMA_eural'] = eural_sbi['LMA_eural'].astype(str).str.zfill(6)

    if level == 'AG':
        eural_sbi['KvK_nace'] = eural_sbi['KvK_ag']
        nace_ewc['NACE'] = nace_ewc['AG']
    elif level == '2':
        eural_sbi['KvK_nace'] = eural_sbi['KvK_sbi'].astype(str).str.slice(stop=2)
        nace_ewc['NACE'] = nace_ewc['NACE'].astype(str).str.slice(stop=2)
    elif level == '4':
        eural_sbi['KvK_nace'] = eural_sbi['KvK_sbi']

    n = len(eural_sbi.drop_duplicates(subset=['LMA_eural', 'KvK_nace']).index)
    print(f'{n} unique EWC-NACE combinations at NACE level {level}')

    nace_ewc = nace_ewc[['EuralCode', 'NACE']].drop_duplicates()
    validation = pd.merge(eural_sbi, nace_ewc, left_on=['LMA_eural', 'KvK_nace'],
    ...
    print(f'{100 - v_ratio}% of all NACE-EWC combinations at NACE level {level} are invalid')
    ...
    validation.to_excel(f'Private_data/validation_{level}.xlsx')
